...
while(1):
    ...
    while(plotCounter<10):
            ...
            if (with_task_id == 1):
                dataBufferPlot[plotCounter] = message[:UDP_BUFFER_SIZE+1]
            else:
                dataBufferPlot[plotCounter] = message[:UDP_BUFFER_SIZE]
    ...
    xs = [j for j in range(1 + (UDP_BUFFER_SIZE * 100 * dataCounter), (UDP_BUFFER_SIZE * 100) + (UDP_BUFFER_SIZE * 100 * dataCounter) + 1)]
    dataCounter += 1


    if (testPerfor == 1):
        test_performance(dataBufferPlot, testCounter)
        testCounter += 1
    else:
        animate(xs, dataBufferPlot, counter, xs_plot)                  #100 * 4096 bytes for 2 channels
        counter += 1
