				...
				when 5 =>
					...
					if stm32_counter < (NUM_WORDS - 1) then
						...
						--temp_buffer(TOTAL_BITS - (stm32_counter*16) - 1 downto TOTAL_BITS - ((stm32_counter+1)*16)) <= std_logic_vector(to_unsigned(RHD_Interval_Counter, 16)) or x"0001"; -- set LSB
							temp_buffer(TOTAL_BITS - (stm32_counter*32) - 1 downto TOTAL_BITS - ((stm32_counter+1)*32)) <= int_FIFO_RHD2132_Q(31 downto 0) or x"00000001"; -- set LSB
						else
							temp_buffer(TOTAL_BITS - (stm32_counter*32) - 1 downto TOTAL_BITS - ((stm32_counter+1)*32)) <= int_FIFO_RHD2132_Q(31 downto 0) and x"FFFFFFFE"; -- clear LSB
						end if;	
