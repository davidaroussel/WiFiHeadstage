...
void initFIFO_byte(FIFO_STRUCTURE_BYTE * p_fifo, int32_t p_fifo_size)
{
  p_fifo->head = 0;
  p_fifo->tail = 0;
  p_fifo->elements_in_fifo = 0;
  p_fifo->availlable_space = p_fifo_size;
...
{
	if((p_fifo->tail + 1 == p_fifo->head) || ((p_fifo->tail + 1 == p_fifo->fifo_size) && (p_fifo->head == 0)))
		return 1;
	return 0;
}
